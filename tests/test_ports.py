...
from tests.support import FakeAPIError, FakeNotFound, load_cm
...
class PortLookupTests(unittest.TestCase):
    ...
    def run_cmd_list(self, client):
        ...
        try:
            ...
            with contextlib.redirect_stdout(stdout):
                result = self.cm.cmd_list(types.SimpleNamespace())
        finally:
    ...
    def test_parse_status_extracts_healthy_healthcheck(self):
        self.assertEqual(
            self.cm.parse_status("Up 2 minutes (healthy)"),
            ("2 minutes", "healthy"),
        )

    def test_parse_status_extracts_unhealthy_healthcheck(self):
        self.assertEqual(
            self.cm.parse_status("Up 2 minutes (unhealthy)"),
            ("2 minutes", "unhealthy"),
        )

    def test_parse_status_extracts_starting_healthcheck(self):
        self.assertEqual(
            self.cm.parse_status("Up 4 seconds (health: starting)"),
            ("4 seconds", "starting"),
        )

    def test_parse_status_ignores_non_up_statuses(self):
        self.assertEqual(
            self.cm.parse_status("Restarting (1) 5 seconds ago"),
            ("-", "-"),
        )
    ...
    def test_summary_port_uses_actual_public_port(self):
        ...
        self.assertEqual(self.cm.get_ssh_port_from_summary(summary), 2301)
    ...
    def test_summary_port_ignores_invalid_public_port(self):
        ...
        self.assertIsNone(self.cm.get_ssh_port_from_summary(summary))
    ...
    def test_container_attrs_use_network_settings_port(self):
        ...
        self.assertEqual(self.cm.get_ssh_port_from_attrs(attrs), 2401)
    ...
    def test_container_attrs_fall_back_to_host_config_port(self):
        ...
        self.assertEqual(self.cm.get_ssh_port_from_attrs(attrs), 2501)

    def test_container_port_returns_fallback_when_mapping_is_missing(self):
        container = FakeContainer(attrs={"NetworkSettings": {"Ports": {}}})

        self.assertEqual(self.cm.get_container_ssh_port(container, 2201), 2201)
    ...
    def test_list_port_inspects_when_summary_mapping_is_missing(self):
        ...
        self.assertEqual(self.cm.get_list_ssh_port(client, summary, 2201), 2601)
    ...
    def test_list_port_uses_fallback_when_inspect_fails(self):
        ...
        self.assertEqual(self.cm.get_list_ssh_port(client, summary, 2201), 2201)
    ...
    def test_cmd_list_prints_stale_image_status(self):
        ...
        self.assertEqual(client.images.names, [self.cm.CM_IMAGE_REF])
    ...
    def test_cmd_ssh_uses_actual_container_port(self):
        ...
        client = FakeClient(container=container)
        original_get_client = self.cm.get_client
        original_execlp = self.cm.os.execlp
        self.cm.get_client = lambda: client
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.cm.get_client = original_get_client
                self.cm.os.execlp = original_execlp
    ...
    def test_port_allocation_error_matches_docker_desktop_message(self):
        ...
        self.assertTrue(self.cm.is_port_allocation_error(error))
